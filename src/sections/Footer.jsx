...
<div className="flex flex-col items-start">
  ...
  <p className="mt-6 text-base leading-7 font-montserrat text-white-400 sm:max-2-sm">Get shoes ready for the new term at your nearest Nike store.
      ...
      {footerLinks.map((section)=>(
        <div key={section}>
          ...
          <ul>{section.links.map((link)=>(
            ...
            leading-normal hover:text-slate-gray cursor-pointer" key={link.name}><a>{link.name} </a></li>
          ))}</ul>
        </div>
      ))}
    </div>
